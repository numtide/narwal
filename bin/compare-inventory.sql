...
-- Stage 2: Extract hashes from inventory parquet files to a temporary table (with last_modified)
CREATE TABLE inventory_hashes ENGINE = MergeTree() ORDER BY hash AS
SELECT
    assumeNotNull(replaceOne(key, '.narinfo', '')) AS hash,
    last_modified_date
...
-- Stage 3: Find inventory entries not in buildstepoutputs and output to file
SELECT i.hash, i.last_modified_date
FROM inventory_hashes i
LEFT ANTI JOIN buildstep_hashes b ON i.hash = b.hash
INTO OUTFILE '/home/brian/Development/com/github/numtide/narwal/.data/inventory-not-in-buildstepoutputs.csv'
FORMAT CSVWithNames;
